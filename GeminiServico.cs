...
{
    /// <summary>Integração do Assistente com o modelo de IA generativa (Gemini) — só texto.</summary>
    public class GeminiServico
    {
        private static readonly HttpClient _http = new HttpClient();

        // Timeout curto pra não deixar a tela "travada" em "Gerando..." esperando o timeout padrão do HttpClient (100s).
        private static readonly TimeSpan TimeoutRequisicao = TimeSpan.FromSeconds(30);

        /// <summary>Envia a requisição com timeout próprio e traduz o cancelamento numa mensagem amigável.</summary>
        ...
        /// <summary>Envia o histórico da conversa pra IA; retorna o texto e o novo nível sugerido (null fora do modo Conversação).</summary>
            ...
            ModoAssistente modo = ModoAssistente.Duvidas, string? nivelAtual = null)
        {
            ...
            bool ehConversaLivre = modo == ModoAssistente.Conversacao;
        ...
        /// <summary>Tira a tag oculta "[NIVEL:xxx]" do final da resposta e devolve o nível normalizado (sem acento).</summary>
        ...
        /// <summary>Traduz uma palavra/frase avulsa (Inglês↔Português), sem usar o histórico da conversa.</summary>
        ...
        // ── GERADOR DE PALAVRAS (resposta em Markdown, não JSON) ──
        ...
        /// <summary>
        /// Gera o conteúdo de uma palavra (Markdown). A primeira linha vira o campo Significado,
        /// o resto vira o campo Exemplo. Não salva nada — só retorna pra revisão no formulário.
        /// </summary>
        public async Task<Palavra> GerarPalavraAsync(ConfiguracaoAssistente cfg, string palavraTexto)
        {
            ...
            // Segurança extra caso a IA ainda inclua algum resquício de markdown.
            textoGerado = LimparMarkdown(textoGerado);

            // Primeira linha ("palavra = significado") → Significado; resto → Exemplo.
        ...
        /// <summary>Gera exercícios de múltipla escolha pra praticar uma palavra já cadastrada.</summary>
        ...
        /// <summary>Remove separadores de markdown ("---", "___", "***") e o "#" de linhas de título.</summary>
        private static string LimparMarkdown(string texto)
        {
            ...
            // Reduz as linhas em branco resultantes pra no máximo uma entre parágrafos.
